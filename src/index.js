...
var makeObjectsSafe = (item, parent) => _.defaults({
  data: {},
  config: {},
  _meta: {
    requests: []
...
var runTypeProcessor = _.curry((getProvider, processor, item, ...args) => Promise.try(() => {
  ...
  throw new Error(`Failed running search for ${item.type} (${item.key}) at ${processor}: ${error}`)
}))
...
module.exports = _.curryN(2, function({
  ...
  return processStep([
      makeObjectsSafe,
      materializePaths,
      item => runProcessor('hasValue', item).then(hasValue => {
    ...
    ]).then(() => processStep(item => {
      // Skip groups
      if (!getItems(item))
        item._meta.relevantFilters = getRelevantFilters(getProvider(item).groupCombinator, item._meta.path, group)
    })).then(() => processStep(item => {
      return runProcessor('validContext', item).then(validContext => {
        // Reject filterOnly
        if (item.filterOnly || !validContext)
          return

        var schema = getSchema(item.schema)
        var curriedSearch = _.partial(getProvider(item).runSearch, [options, item, schema, item._meta.relevantFilters])
        return runProcessor('result', item, curriedSearch, schema, getProvider(item), options).catch(error => {
      ...
      if (!options.debug)
        delete item._meta
    })).return(group).catch(error => {
      throw error.item ? error : new Error(`Failed running search (uncaught): ${error}`)
    })
    // .then(() => {
    //     console.log('------------')
